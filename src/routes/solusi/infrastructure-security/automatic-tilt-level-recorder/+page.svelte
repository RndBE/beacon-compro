<script lang="ts">
	...
	const consultUrl = $derived(
		`https://wa.me/628112632151?text=${encodeURIComponent(
			$locale === 'EN'
				? 'Hello Beacon Marketing CS, I would like to consult about ATLR (Automatic Tilt Level Recorder).'
				: 'Halo CS Marketing Beacon, saya ingin konsultasi tentang ATLR (Automatic Tilt Level Recorder).'
		)}`
	);
