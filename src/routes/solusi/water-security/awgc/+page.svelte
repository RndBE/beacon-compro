<script lang="ts">
	...
	const mainContent = $derived(
		ss?.main_content?.trim() ||
			'<p>Buka-tutup pintu air dari smartphone Anda. Tidak perlu lagi operator turun ke lapangan saat hujan deras tengah malam. Auto-leveling, fail-safe, audit-ready.</p>'
	);
	...
	const copy = $derived(
		$locale === 'EN'
			? {
					...
					eyebrow: 'Water Security · Remote Control',
					title1: 'Control the gate,', title2: 'from anywhere.',
					consult: 'Consult AWGC', other: 'Other Solutions', live: 'LIVE',
					fig: 'FIG.05 — WATER GATE', gate: 'GATE OPENING', mode: 'MODE', up: 'UPSTREAM', down: 'DOWNSTREAM', auto: 'AUTO',
					chainBadge: 'Signal Chain', chainTitle: 'From a tap on your phone to the gate leaf',
					chainDesc: 'Level and gate-position are read continuously; commands flow back over RS485 / 4G to the actuator — so a gate moves the moment it must, day or night.',
					capBadge: 'Capabilities', capTitle: 'Move water without leaving the office',
					appBadge: 'Where it works', appTitle: 'For every gate that must answer fast',
					appDesc: 'Dam gates, irrigation networks, spillways, river weirs, and urban drainage.',
					trackBadge: 'Track Record', trackTitle: 'Already deployed at',
					ctaBadge: 'Next Step', ctaTitle: 'Automate your gates with AWGC',
					ctaDesc: 'Our engineers will design the right actuator setup and integrate it into your STESY dashboard.',
					ctaPrimary: 'Consult AWGC', ctaSecondary: 'Explore Other Products'
				}
			: {
					home: 'Beranda', solutions: 'Solusi', parent: 'Water Security',
					eyebrow: 'Water Security · Kontrol Jarak Jauh',
					title1: 'Kendalikan pintu air,', title2: 'dari mana saja.',
					consult: 'Konsultasi AWGC', other: 'Solusi Lain', live: 'LIVE',
					fig: 'FIG.05 — PINTU AIR', gate: 'BUKAAN PINTU', mode: 'MODE', up: 'HULU', down: 'HILIR', auto: 'AUTO',
					chainBadge: 'Rantai Sinyal', chainTitle: 'Dari satu ketukan di layar ke daun pintu',
					chainDesc: 'Level dan posisi pintu dibaca terus-menerus; perintah mengalir balik lewat RS485 / 4G ke aktuator — pintu bergerak tepat saat dibutuhkan, siang atau malam.',
					capBadge: 'Kemampuan', capTitle: 'Atur air tanpa meninggalkan kantor',
					appBadge: 'Penerapan', appTitle: 'Untuk setiap pintu yang harus sigap',
					appDesc: 'Pintu bendungan, jaringan irigasi, spillway, bendung sungai, dan drainase perkotaan.',
					trackBadge: 'Track Record', trackTitle: 'Sudah Terpasang Di',
					ctaBadge: 'Langkah Selanjutnya', ctaTitle: 'Otomatiskan pintu air Anda dengan AWGC',
					ctaDesc: 'Tim engineer kami akan merancang setup aktuator yang tepat dan mengintegrasikannya ke dashboard STESY.',
					ctaPrimary: 'Konsultasi AWGC', ctaSecondary: 'Jelajahi Produk Lain'
				}
	);
	...
	const consultUrl = $derived(
		`https://wa.me/628112632151?text=${encodeURIComponent(
			$locale === 'EN'
				? 'Hello Beacon Marketing CS, I would like to consult about AWGC (Automatic Water Gate Controller).'
				: 'Halo CS Marketing Beacon, saya ingin konsultasi tentang AWGC (Automatic Water Gate Controller).'
		)}`
	);

	// ── Live water gate ──
	let mounted = $state(false);
	let gateOpen = $state(0); // percent open
	let upstream = $state(0);
	let downstream = $state(0);

	onMount(() => {
		mounted = true;
		const reduce = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
		const tick = () => {
			gateOpen = Math.round((28 + Math.random() * 54) * 10) / 10; // 28–82 %
			upstream = Math.round((3.2 + Math.random() * 1.6) * 100) / 100;
			downstream = Math.round((1.1 + Math.random() * 1.2) * 100) / 100;
		};
		tick();
		if (reduce) return;
	...
	const chain = [
		{ icon: Gauge, k: '01', t: $locale === 'EN' ? 'Level + Position' : 'Level + Posisi', d: $locale === 'EN' ? 'Sensor feedback' : 'Umpan-balik sensor' },
		{ icon: Settings, k: '02', t: $locale === 'EN' ? 'Actuator' : 'Aktuator', d: $locale === 'EN' ? 'Motor / hydraulic' : 'Motor / hidraulik' },
		{ icon: Radio, k: '03', t: $locale === 'EN' ? 'Transmission' : 'Transmisi', d: '4G/LTE · GSM' },
		{ icon: LayoutDashboard, k: '04', t: 'STESY', d: 'Dashboard' }
	];

	const capabilities = $derived([
		{ icon: Zap, value: '<2', unit: 's', label: $locale === 'EN' ? 'Command response' : 'Respons perintah' },
		{ icon: ShieldCheck, value: 'Fail', unit: 'safe', label: $locale === 'EN' ? 'Auto-lock on signal loss' : 'Auto-lock saat sinyal putus' },
		{ icon: Settings, value: '500', unit: 'Nm', label: $locale === 'EN' ? 'Maximum torque' : 'Torque maksimal' },
		{ icon: Wifi, value: 'Remote', unit: '', label: $locale === 'EN' ? 'STESY integration' : 'Integrasi STESY' }
	]);

	const applications = $derived([
		{ icon: Waves, label: $locale === 'EN' ? 'Dam Gates' : 'Pintu Bendungan' },
		{ icon: Droplets, label: $locale === 'EN' ? 'Irrigation' : 'Irigasi' },
		{ icon: Activity, label: 'Spillway' },
		{ icon: Gauge, label: $locale === 'EN' ? 'River Weirs' : 'Bendung Sungai' },
		{ icon: Factory, label: $locale === 'EN' ? 'Urban Drainage' : 'Drainase Perkotaan' }
	]);
</script>
...
<div class="border-b" style="background: #100A0B; border-color: rgba(200,16,46,0.18);">
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
		<nav class="flex items-center gap-1.5 text-xs font-mono" style="color: #8a767a;">
			...
			<ChevronRight size={11} />
			<span class="font-semibold" style="color: {ACCENT_SOFT};">AWGC</span>
		</nav>
	</div>
</div>
...
<section class="relative overflow-hidden" style="background: #0C0809;">
	<div class="awgc-grid absolute inset-0 pointer-events-none"></div>
		...
		<div class="grid lg:grid-cols-[1.05fr_0.95fr] gap-14 lg:gap-10 items-center">
			<div>
				...
				<h1 class="font-heading font-extrabold tracking-tighter text-white leading-[0.98] text-5xl sm:text-6xl lg:text-[64px]" style="opacity: {mounted ? 1 : 0}; transform: translateY({mounted ? 0 : 18}px); transition: all .8s cubic-bezier(.16,1,.3,1) .15s;">
					{copy.title1}<br /><span class="awgc-gradient">{copy.title2}</span>
				</h1>
				<div class="mt-7 flex items-baseline gap-3" style="opacity: {mounted ? 1 : 0}; transition: opacity .8s ease .3s;">
					<span class="font-heading text-2xl font-bold" style="color: {ACCENT_SOFT};">AWGC</span>
					<span class="text-sm text-zinc-500 font-mono">Automatic Water Gate Controller</span>
				</div>
				<div class="awgc-lead mt-5 max-w-[46ch] text-lg leading-relaxed text-zinc-400" style="opacity: {mounted ? 1 : 0}; transform: translateY({mounted ? 0 : 12}px); transition: all .8s cubic-bezier(.16,1,.3,1) .4s;">
					{@html mainContent}
				</div>
			...
			<div class="relative flex justify-center lg:justify-end" style="opacity: {mounted ? 1 : 0}; transform: scale({mounted ? 1 : 0.94}); transition: all .9s cubic-bezier(.16,1,.3,1) .35s;">
				<div class="awgc-instrument relative w-[360px] max-w-full rounded-[28px] p-6">
					...
					<div class="flex items-stretch gap-5">
						<!-- gate frame -->
						<div class="awgc-frame">
							<div class="awgc-leaf" style="height: {100 - gateOpen}%;">
								<span class="awgc-rib" style="top: 22%;"></span>
								<span class="awgc-rib" style="top: 48%;"></span>
								<span class="awgc-rib" style="top: 74%;"></span>
							</div>
							<div class="awgc-flow" style="height: {gateOpen}%;"></div>
							{#each [25, 50, 75] as g}
								<div class="awgc-tick" style="bottom: {g}%;"></div>
							{/each}
						</div>
						<!-- readouts -->
						<div class="flex-1 flex flex-col justify-center gap-3">
							<div class="awgc-readout">
								<span class="font-mono text-[9px] tracking-[0.2em] text-zinc-500">{copy.gate}</span>
								<span class="font-mono text-xl font-bold tabular-nums" style="color: {ACCENT_SOFT};">{gateOpen.toFixed(1)}<span class="text-xs text-zinc-500"> %</span></span>
							</div>
							<div class="awgc-readout">
								<span class="font-mono text-[9px] tracking-[0.2em] text-zinc-500">{copy.mode}</span>
								<span class="inline-flex items-center gap-1.5 font-mono text-sm font-bold" style="color: {LIVE};">
									<span class="w-1.5 h-1.5 rounded-full" style="background: {LIVE};"></span>{copy.auto}
								</span>
							</div>
							<div class="grid grid-cols-2 gap-2.5">
								<div class="awgc-readout">
									<span class="font-mono text-[8px] tracking-[0.16em] text-zinc-500">{copy.up}</span>
									<span class="font-mono text-sm font-bold tabular-nums text-white">{upstream.toFixed(2)}<span class="text-[9px] text-zinc-500"> m</span></span>
								</div>
								<div class="awgc-readout">
									<span class="font-mono text-[8px] tracking-[0.16em] text-zinc-500">{copy.down}</span>
									<span class="font-mono text-sm font-bold tabular-nums text-white">{downstream.toFixed(2)}<span class="text-[9px] text-zinc-500"> m</span></span>
								</div>
							</div>
						</div>
					</div>
...
<section class="relative py-20 lg:py-24 overflow-hidden" style="background: #130A0C;">
	<div class="awgc-grid absolute inset-0 pointer-events-none opacity-60"></div>
...
<!-- PRODUCT — kept as-is -->
<ProductVariantDossier {variants} productName="AWGC" />
...
<section class="relative py-20 bg-white">
	<div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
		<div class="relative overflow-hidden rounded-[2.5rem] p-10 sm:p-16 lg:p-20 flex flex-col lg:flex-row items-center justify-between gap-12 group" style="background: #1A0A0D; box-shadow: 0 40px 80px -24px rgba(26,10,13,0.55);">
			<div class="awgc-grid absolute inset-0 pointer-events-none opacity-50"></div>
...
<style>
	.awgc-gradient { background: linear-gradient(100deg, #FF4D6D 0%, #C8102E 55%, #A50D25 100%); -webkit-background-clip: text; background-clip: text; color: transparent; }
	.awgc-lead :global(p) { margin: 0; }
	.awgc-grid {
	...
	.awgc-leaf {
		...
		background: linear-gradient(180deg, #5a0a16 0%, #C8102E 60%, #8c0c20 100%);
		border-bottom: 2px solid rgba(255,200,210,0.85);
		box-shadow: 0 3px 16px rgba(200,16,46,0.5);
		transition: height 2.2s cubic-bezier(0.25,0.8,0.3,1);
	}
	.awgc-rib { position: absolute; left: 14%; right: 14%; height: 1px; background: rgba(255,255,255,0.14); }
	.awgc-flow {
		position: absolute;
		left: 0; right: 0; bottom: 0;
		background: linear-gradient(180deg, rgba(255,77,109,0.22) 0%, rgba(200,16,46,0.38) 100%);
		overflow: hidden;
		transition: height 2.2s cubic-bezier(0.25,0.8,0.3,1);
	}
	.awgc-flow::after {
		...
		background: repeating-linear-gradient(118deg, transparent 0 7px, rgba(255,210,220,0.16) 7px 9px);
		animation: awgc-stream 0.7s linear infinite;
	}
	@keyframes awgc-stream { from { transform: translateX(0); } to { transform: translateX(-13px); } }
	.awgc-tick { position: absolute; left: 0; width: 7px; height: 1px; background: rgba(255,255,255,0.18); }
	.awgc-readout { display: flex; flex-direction: column; gap: 2px; border-radius: 12px; padding: 10px 12px; background: rgba(255,255,255,0.03); border: 1px solid rgba(200,16,46,0.16); }
	@media (prefers-reduced-motion: reduce) { .awgc-leaf, .awgc-flow { transition: none; } .awgc-flow::after { animation: none; } }
</style>
