<script lang="ts">
	...
	const consultUrl = $derived(
		`https://wa.me/628112632151?text=${encodeURIComponent(
			$locale === 'EN'
				? 'Hello Beacon Marketing CS, I would like to consult about AASM (Automatic Agriculture Smart Monitoring).'
				: 'Halo CS Marketing Beacon, saya ingin konsultasi tentang AASM (Automatic Agriculture Smart Monitoring).'
		)}`
	);
