...
(() => {
  ...
  const stylesheet = `
    ...
    :host([data-interactive-slide]) .tapzones { display: none; }
    @media (hover: none), (pointer: coarse) {
      :host([data-interactive-slide]) .overlay {
        opacity: 1;
        pointer-events: auto;
    ...
    .btn[aria-pressed="true"] { color: #fff; background: rgba(255,255,255,0.10); }
  ...
  class DeckStage extends HTMLElement {
    ...
    _render() {
      ...
      overlay.innerHTML = `
        ...
        <span class="divider"></span>
        <button class="btn railtoggle" type="button" aria-label="Toggle sidebar" title="Hide sidebar (S)" aria-pressed="true">
          <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2.5" y="3" width="11" height="10" rx="1.5"/><line x1="6.5" y1="3" x2="6.5" y2="13"/></svg>
        </button>
        <button class="btn fs" type="button" aria-label="Toggle fullscreen" title="Fullscreen (F)" aria-pressed="false">
          <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 3H3v3"/><path d="M10 3h3v3"/><path d="M6 13H3v-3"/><path d="M10 13h3v-3"/></svg>
        </button>
      `;
      ...
      overlay.querySelector('.railtoggle').addEventListener('click', () => this._setRailVisible(this._railVisible === false));
      ...
      this._railBtn = overlay.querySelector('.railtoggle');
    ...
    _applyIndex({ showOverlay = true, broadcast = true, reason = 'init' } = {}) {
      ...
      const interactive = !!(activeSlide && activeSlide.querySelector('[data-component="map"], .leaflet-container, iframe, video, input, textarea, select, button, a[href]'));
      if (interactive) this.setAttribute('data-interactive-slide', '');
      else this.removeAttribute('data-interactive-slide');
    ...
    _railWidth() {
      ...
      if (!this._railEnabled || !this._railVisible || this.hasAttribute('no-rail')
          ...
          || this._isCompactViewport()) return 0;
    ...
    _syncRailHidden() {
      ...
      const hard = !this._railEnabled || this._presenting || this._previewMode || compact;
    ...
    /** Per-viewer rail show/hide. Shared by the overlay sidebar button,
     *  the S key, and the host's __deck_rail_visible message. */
    _setRailVisible(on) {
    ...
    _onFsChange() { this._syncChromeToggles(); }

    /** Reflect fullscreen + rail state onto the overlay toggle buttons
     *  (aria-pressed + tooltip). Safe before the overlay exists. */
    _syncChromeToggles() {
      const fsOn = !!(document.fullscreenElement || document.webkitFullscreenElement);
      if (this._fsBtn) {
        this._fsBtn.setAttribute('aria-pressed', fsOn ? 'true' : 'false');
        this._fsBtn.title = fsOn ? 'Exit fullscreen (F)' : 'Fullscreen (F)';
      }
      if (this._railBtn) {
        const vis = this._railVisible !== false;
        this._railBtn.setAttribute('aria-pressed', vis ? 'true' : 'false');
        this._railBtn.title = vis ? 'Hide sidebar (S)' : 'Show sidebar (S)';
      }
    }
    ...
    _onKey(e) {
      ...
      if (key === 'ArrowRight' || key === 'PageDown' || key === ' ' || key === 'Spacebar') {
        ...
        this._setRailVisible(this._railVisible === false);
      } else {
        handled = false;
      }
